...
**Accepted (Provisional, Validation Gate: C0 PASS 2026-04-30 / C1 — pending ADR-0002 V1 + Class-Switch prototype)**

> 本 ADR は **thin provisional** scope で起草された。`/autoplan` Phase 4 USER CHALLENGE で CEO + Eng 両モデル独立 dual voice review が 8 cross-phase systemic themes を flag した結果、ユーザは Option B（Locked Decisions 3 件 + Deferred Decisions 11 件 + R1 spike 前提）を採択。本 ADR は Cinemachine 3 採用 / ICharacterMotor.Position follow / 単一 PolygonCollider2D Confiner の 3 点のみを lock し、Body component 名 / Damping / Foundation Singleton 適用 / Pixel Perfect Reference Resolution / forbidden patterns / Look Ahead 等 11 件は **R1 spike 通過 + ADR-0002 V1 通過 + Class-Switch prototype 実行** 後の follow-up ADR で empirical data ベースで lock する。
>
> **C0 PASS (2026-04-30)**: Unity 6.3 LTS (6000.3.13f1) + Cinemachine 3.1.6 + URP 17.0.3 で 12/12 検証 OK、weighted score **18/18**（Critical 3 件 #8/#9/#11 すべて PASS）。Cinemachine 3.x API surface 前提は成立、3 Locked Decisions は維持。Evidence: [`production/qa/evidence/r1-camera-cinemachine3-spike-result.md`](../../production/qa/evidence/r1-camera-cinemachine3-spike-result.md). 発見事項（CinemachinePixelPerfect functional / `UseSignalSpaceOnly`→`UseCameraSpace` rename / `CinemachineBrain` no `[DefaultExecutionOrder]` 等）は **ADR-0006a (follow-up)** で反映。C1 (Provisional Follow Basic) は **ADR-0002 V1 通過後** に検証。
...
- **C0 — R1 Spike Completion** ✅ **PASS 2026-04-30**: Unity 6.3 LTS (6000.3.13f1) + Cinemachine 3.1.6 + URP 17.0.3 で 12/12 verification items 検証完了、weighted score **18/18**（Critical 3 件 #8/#9/#11 すべて PASS）。Evidence: [`production/qa/evidence/r1-camera-cinemachine3-spike-result.md`](../../production/qa/evidence/r1-camera-cinemachine3-spike-result.md). Spike: [`prototypes/camera-cinemachine3-r1-spike/`](../../prototypes/camera-cinemachine3-r1-spike/). 発見事項（CinemachinePixelPerfect functional / `UseSignalSpaceOnly`→`UseCameraSpace` rename / `CinemachineBrain` に `[DefaultExecutionOrder]` 属性なし / `BoundingShape2D` は field 公開 等）は **ADR-0006a (follow-up)** で反映、本 ADR の 3 Locked Decisions は影響なし
- **C1 — Provisional Follow Basic**（ADR-0002 V1 通過後、未実施）: motor.Position drives camera follow within 1 render frame、Brain.UpdateMethod を architecture で pin（spike #2 結果: SmartUpdate default 確認済）、30/60/120Hz render × 50Hz physics matrix で sync 検証、`Time.timeScale=0.1` slow-mo + hitstop+dash 組合せで 1-frame divergence ≤ 0.005 unit p99（PlayMode test）。**動的追従 stutter** + **render-space pixel snap jitter** 検証も C1 scope に含める（C0 spike は static-state 検証のみ、CD-PLAYTEST CONCERN C 反映）
