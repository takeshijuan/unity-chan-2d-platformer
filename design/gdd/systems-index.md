...
- `ISaveable` interface + section-based JSON（player / world / settings / meta セクション）
- 論理分割は実装内部で達成
...
1. **Input System** — Unity Input System の薄い service ラッパ（Action Rebinding / Steam Input / コンボバッファ向け timestamp 配信）。これがないと何も操作できない
